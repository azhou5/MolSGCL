...
def get_plausibility(description_list, smiles_list, original_smiles, original_molecule_description, task_description, dataset_description):
    ...
    content = f"""You are an expert medicinal chemist evaluating molecules that are {task_description}s, and determining which substructure explains this fact. You have 
     received several substructures of the molecule. You will be provided the description of each of these substructures as well 
      ...
      You are responsible for determining which of the provided substructures is most likely to be the core element of the molecule that explains its properties
       and which is the least likely to be the core element of the molecule that explains its properties. 
        ...
        messages=[
            {
                ...
                If it is not clear which substructure is the best or worst rationale, you should return a list of 0s. [0,0,0,...0]. Do not hesitate to return a list of 0s if you are not sure or if the plausible and implausible substructures are very similar. 
                If you only have the worst rationale, return -1 in that position and 0s for all other positions. 
                Thus, your output should either be a list with a single 1 and a single -1, and the rest 0s, or a list of 0s, or a list of zeros with a single -1. No other format is allowed. Please make sure that the plausible and implausible substructures are distinct from each other functionally and would be sensical to guide a triplet loss. They should not have the exact same functional groups, if they do return all 0s. 
